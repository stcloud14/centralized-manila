...
const LCRCardView = ({ filteredBirthCert, filteredDeathCert, filteredMarriageCert, handleModalOpen, handleRejectConfirm, handleProcessConfirm, handleCompleteConfirm, section }) => {
  ...
  return (
    ...
    {filteredMarriageCert && filteredMarriageCert.length > 0 && filteredMarriageCert.map((transaction) => (
      <div onClick={(e) => handleModalOpen(transaction, 'Marriage Certificate', e)} key={transaction.transaction_id} className="cursor-pointer bg-white dark:bg-[#333333] shadow-[0_4px_10px_-1px_rgba(0,0,0,0.14)] dark:shadow-[0_4px_10px_-1px_rgba(0,0,0,0.2)] rounded-sm flex flex-col">
        ...
        <div className="flex-grow px-4 pt-5 pb-4">
        <div className="text-xs text-slate-600 dark:text-slate-300 my-1">Type: {transaction.trans_type} </div>
          <div className="text-xs text-slate-600 dark:text-slate-300 my-1">First Name: {transaction.husband_fname} </div>
          <div className="text-xs text-slate-600 dark:text-slate-300 my-1">Last Name: {transaction.husband_lname}  </div>
          <div className="text-xs text-slate-600 dark:text-slate-300 my-1">Date Processed: {transaction.date} </div>
          <div className="text-xs text-slate-600 dark:text-slate-300 my-1">Time Processed: {transaction.time} </div>
          <div className="flex justify-start items-center text-xs text-slate-600 dark:text-slate-300 my-1">
            <span>Status: {transaction.status_type} </span>
