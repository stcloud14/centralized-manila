...
const getStatusColors = (statusType) => {
  switch (statusType) {
    ...
    case 'Processing':
      return { bgColor: 'bg-yellow-200', textColor: 'text-yellow-800', text: <p>The transaction has been processing.</p> };
    case 'Complete':
...
const StatusBadgeModal = ({ statusType }) => {
  ...
  return (
    <div className="flex relative" ref={popoverRef} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <span className={`text-[0.65rem] flex justify-center py-0.5 pl-1 rounded-full ${bgColor} ${textColor} w-24`}>
