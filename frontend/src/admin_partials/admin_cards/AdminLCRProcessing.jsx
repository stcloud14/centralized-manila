...
const AdminLCRProcessing = ({ birthCert, deathCert, marriageCert, handleUpdateData }) => {
  ...
  const handleSearch = () => {
    ...
    let filteredDC = deathCert.filter(transaction => {
      ...
      let isTypeMatch = !selectedType || selectedType === 'All' || parseInt(selectedType) === 0 ||
        (selectedType === 'Birth Certificate' && transaction.trans_type === 'Birth Certificate') ||
        (selectedType === 'Death Certificate' && transaction.trans_type === 'Death Certificate') ||
        (selectedType === 'Marriage Certificate' && transaction.trans_type === 'Marriage Certificate');
  ...
  const handleComplete = async () => {  
    ...
    try {
      const response = await axios.post(`${Base_Url}adminlcr/updatecomplete/${transaction_id}/${admin_uname}`, selectedTransaction);
      setIsLoading(true);
      ...
      <>
        ...
        <div className="flex flex-col col-span-full sm:col-span-6 bg-white dark:bg-[#2b2b2b] dark:border-[#3d3d3d] shadow-[0_4px_6px_-1px_rgba(0,0,0,0.1)] dark:shadow-[0_4px_6px_-1px_rgba(0,0,0,0.2)] rounded-sm border border-slate-200">
          <div className="px-5 py-5">
            <h1 className='font-medium text-center text-slate-700 dark:text-white mb-4'>Local Civil Registry Processing</h1>
          ...
          <div className="flex flex-col items-center sm:flex-row text-xs pb-5">
            ...
            <div className="relative w-full sm:w-20 text-left z-10">
              ...
              {/* Owner's Name */}
              <div className="flex justify-center sm:justify-between items-center pb-[6px] sm:pb-[8px]">
                <span className="hidden sm:block pr-10 text-xs">Client Name:</span>
                <div className="relative flex items-center">
                  <span className="absolute inset-y-0 left-0 pl-2 flex items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
                      <path className='stroke-slate-400 dark:stroke-white' strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
                  ...
                  <input value={searchOwner} onChange={(e) => setSearchOwner(e.target.value.toUpperCase())} id="searchInput" type="text" placeholder="Search Last Name..." className="bg-transparent text-xs w-[235px] sm:w-[210px] border border-slate-300 text-slate-700 dark:text-white pl-8 py-1 md:py-0.5 rounded-sm"/>
                </div>
              </div>
