...
const AdminRPTaxProcessing = ({ taxPayment, taxClearance, handleUpdateData }) => {
  ...
  const handleComplete = async () => {  
    ...
    try {
      const response = await axios.post(`${Base_Url}adminrptax/updatecomplete/${transaction_id}`, selectedTransaction);
      setIsLoading(true);
                  ...
                  <div className="inline-block align-bottom bg-white rounded-sm text-center overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    ...
                    <div className="bg-white dark:bg-[#212121] px-4 py-3 gap-3 sm:px-6 flex justify-end">
                      ...
                      <button
                        ...
                        type="button"
                        className="text-white text-xs md:text-sm bg-emerald-500 border border-emerald-500 hover:border-emerald-600 hover:bg-emerald-600 focus:ring-4 focus:outline-none focus:ring-emerald-300 font-normal rounded-full px-5 py-2 text-center mb-2 dark:border-emerald-500 dark:text-white dark:hover:text-white dark:hover:bg-emerald-700 dark:hover:border-emerald-700 dark:focus:ring-emerald-800"
                        >
                      Confirm
                      </button>
                    </div>
