...
const router = Router();
  ...
  router.delete('/accdelete/:user_id', async (req, res) => {
    ...
    const sql1 = "DELETE FROM user_reg WHERE user_id = ?";
    const sql2 = "DELETE FROM user_auth WHERE user_id = ?";
    ...
    const sql6 = "DELETE FROM user_birth WHERE user_id = ?";
    const sql7 = "DELETE FROM user_image WHERE user_id = ?";
    const sql8 = "DELETE FROM user_verification WHERE user_id = ?";
    ...
    try {
        ...
        const result8 = await queryDatabase(sql8, [user_id]);
        ...
        res.json({
            ...
            user_birth_result: result6,
            user_image_result: result7,
            user_verification_result: result8,
        });
    } catch (err) {
        console.error(err);
