...
const TaxClearanceModal = ({ user_id, selectedTransaction, onClose, onSubmit, handleOpenModal }) => {
...
const cancelTrans = async (e) => {
  ...
  useEffect(() => {
    const fetchTaxClearanceTransaction = async () => {
      if (transaction_id) {
        try {
          const res = await axios.get(`${Base_Url}transachistory/taxclearance/${transaction_id}`);
  ...
  const generateDownloadLink = (data) => {
    ...
    return `${Base_Url}transachistory/taxclearance/${data.transaction_id}/download`;
};
                    ...
                    <div className="mx-auto">
                      ...
                      <div className="mb-6">
                        ...
                        {taxClearanceTransaction && status_type ? (
                          `P {taxClearanceTransaction.amount ? taxClearanceTransaction.amount + '.00': '-'}`
              ...
              <div className="flex bg-white dark:bg-[#212121] text-slate-700 dark:text-white p-4 rounded-b-lg gap-4 items-end">
                ...
                <div className="flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-2 mt-2 sm:mt-0 ml-auto">
                  {status_type === 'Pending' && transaction_id ? (
                    ...
                    >
                      <span className="font-semibold">PAY: P {taxClearanceTransaction.amount ? taxClearanceTransaction.amount + '.00' : '-'}</span>
                    </button>
                  ) : null}
