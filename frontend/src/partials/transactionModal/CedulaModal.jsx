...
const cancelTrans = async (e) => {
  ...
  useEffect(() => {
    const fetchCedulaTransaction = async () => {
      if (transaction_id) {
        try {
          const res = await axios.get(`${Base_Url}transachistory/cedula/${transaction_id}`);
  ...
  const generateDownloadLink = (data) => {
    ...
    return `${Base_Url}transachistory/cedula/${data.transaction_id}/download`;
};
          ...
          status_type === 'Pending' && (
            ...
            <div className='bg-white dark:bg-[#212121] mb-5 p-5 rounded-lg'>
                      ...
                      <div className="mx-auto">
                        ...
                        {cedulaTransaction.ctc_suffix ? (
                          ...
                          <div className="flex flex-col sm:flex-row items-start justify-between mb-1">
                            ...
                            <span className="whitespace-nowrap md:mb-0 mb-1">{cedulaTransaction.ctc_valididLabel || cedulaTransaction.valid_id_type || '-'}</span>
                          </div>
                          <div className="flex flex-col sm:flex-row items-start justify-between mb-1">
              ...
              <div className="mx-auto bg-white dark:bg-[#212121] text-slate-700 dark:text-white pb-4 pl-4 pr-4 sm:pl-6 sm:pr-6 md:pl-6 md:pr-6 lg:pr-10 ">
                ...
                {transaction_id && cedulaTransaction.reject_cause ? (
                <div className="flex flex-col sm:flex-row items-start justify-between mb-1">
                  <span className="font-medium whitespace-nowrap">Remarks</span>
                  <span className="whitespace-normal text-left md:mb-0 mb-1 text-red-500">{cedulaTransaction.reject_cause}</span>
                </div>
                ) : null}
