...
const BPCardView = ({filteredBusinessPermit, handleModalOpen, handleRejectConfirm, handleProcessConfirm, handleCompleteConfirm, section }) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 pb-4">
        {filteredBusinessPermit.map((transaction) => (
          ...
          <div onClick={(e) => handleModalOpen(transaction, 'Business Permit', e)} key={transaction.transaction_id} className="cursor-pointer bg-white dark:bg-[#333333] shadow-[0_4px_10px_-1px_rgba(0,0,0,0.14)] dark:shadow-[0_4px_10px_-1px_rgba(0,0,0,0.2)] rounded-sm flex flex-col">
            ...
            <div className="flex-grow px-4 pt-5 pb-4">
              <div className="text-xs text-slate-600 dark:text-slate-300 my-1">Business Type:  {transaction.bus_type}</div>
              <div className="text-xs text-slate-600 dark:text-slate-300 my-1">Tax Identification:  {transaction.bus_tin}</div>
