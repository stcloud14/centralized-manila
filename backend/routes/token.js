...
router.get('/protected-route', verifyToken, (req, res) => {
    ...
    res.status(200).json({ message: `Authenticated user with user ID ${user_id}.`, user_id: req.decoded.user_id });
});
...
router.get('/protect-token-admin/:admin_type', verifyTokenForAdmin, (req, res) => {
    ...
    res.status(200).json({ message: `Authenticated user with user ID ${admin_type}.` });
});
